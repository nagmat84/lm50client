...
ReadHoldingRegistersRes::ReadHoldingRegistersRes( const char* rawDatagram, unsigned int length ): Base() {
	// An read holding registers response datagram is at least 9 bytes long.
	...
	dgram = reinterpret_cast< RHRegResDatagram* >( new char[mbLength+6] );
...
ReadHoldingRegistersRes::ReadHoldingRegistersRes( const ReadHoldingRegistersRes& other ): Base( other ) {
	const unsigned int realLength = other.totalLength();
	dgram = reinterpret_cast< RHRegResDatagram* >( new char[ realLength ] );
	memcpy( dgram->rawData, other.dgram->rawData, realLength );
}


ReadHoldingRegistersRes::~ReadHoldingRegistersRes() {
